...
const ListImageResult = ({
  ...
  const handleSetIdxSelect = (idx) => {
    ...
    const newUrls = NewImageArr.map(
      (item, id) =>
        "http://127.0.0.1:7000/" + imageUrls[item[idx].idImg].slice(1)
    );
    setUrlList(newUrls);
  };
  ...
  return (
    <div className="h-full relative">
      ...
      {NewImageArr.length > 0 && (
        <div className="flex flex-row gap-4 mb-2 ">
          {NewImageArr.map((item, idx) => (
            <div
              key={`stage${idx}`}
              className={`flex gap-4 ${stage === idx ? "text-teal-500" : "text-black"
                }`}
            >
              <button
                className={`hover:text-teal-500 ${stage === idx ? "text-teal-500" : "text-black"
                  }`}
      ...
      {NewImageArr.length > 0 && (
        <div
          className={`w-full grid grid-cols-5 overflow-y-scroll mb-2 ${isHidden ? "h-5/6" : "h-4/6"
            }`}
          style={HiddenStyle}
        >
      ...
      <div
        style={HiddenStyle}
        className={`border border-teal-500 p-1 gap-2 overflow-x-auto w-full bottom-12 absolute flex flex-row ${isHidden ? "h-0 border-none" : "h-1/5"
          }`}
      >
        {idxSelect >= 0 &&
          !isHidden &&
