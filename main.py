...
beit3_fea_path = r"/data/vit/AIC2024/DATA/features/beit3_features"
...
retrieval.load_feature(device=device, type_fea="beit3", beit3_fea_path=beit3_fea_path)
...
def handle_text_query(data, K):
    ...
    # print(ids_result, distances)
    return {"ids": ids_result, "distances": distances}

def handle_speech_query(data, K):
    ids_result, distances = retrieval.elastic.Elastic_retrieval(data, K*10, "asr")
    return {"ids": ids_result, "distances": distances}

def handle_llm_query(data, K):
    ids_result, distances = retrieval.llm.retrieve(data,K)
    return {"ids": ids_result, "distances": distances}
