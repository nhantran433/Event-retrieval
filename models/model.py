...
class Event_retrieval():
    ...
    def load_feature(self,  device, type_fea = "all", beit3_fea_path = None, sket_fea_path = None, distance_metric="cosine"):
        if type_fea == "beit3":
            if beit3_fea_path is None:
                raise ValueError("For type_model='beit3', 'beit3_fea_path' must be provided.")
            
            self.beit3.load_feature(beit3_fea_path, distance_metric, device)
    ...
    def temporal_search(self, ids, scores, df_path = r"./DATA/temporal_map.csv"):
            ...
            for i in range(len(id_curr)):
                ...
                for j in range(len(id_next)):
                    ...
                    if (self.mapdata['file_name'][id1] == self.mapdata['file_name'][id2]) and id2 > id1:
                        ...
                        score = score*math.exp(-abs(0.1*(t-3)))
                        if score > max_score:
                            max_score = score
                            id1_max = id1
                            id2_max = id2
